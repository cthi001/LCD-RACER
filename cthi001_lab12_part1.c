...
unsigned menu,pause,lost,ms,fire,shoot,que;etime;lose_time;score_time,gcd,type1,type2,fp,score,yeet_time,del,numplay,pmark;
...
int obs1(state) {
	if (pause ==0) {
		...
		switch (state) {
			case start:break;
			case shift:pos1--;if(pos1 == cnt1){etime2=0;};break;
...
int display(dstate){
	...
	switch(dstate) {
		case start2:dstate=mmenu,menu=1;task5.period =100;pause=1;break;
		case mmenu:if (menu ==0){s1=0;s2=0;s3=0;cnt=1;score =0;pos1=10;pos2=32;task2.period = 300;task3.period=300;pause =0;task5.period=50;
					
					fp=0;dstate = write;break;}
			break;
		case lose: 
				if(lose_time>3000){fp=0;etime=0;dstate= mmenu;task5.period=100;numplay=1;menu=1;pause=1;
					task2.period =300;task3.period=300;endcnt=0;cnt2 = 16;posc=17;index=0;saved=0;LCD_ClearScreen();LCD_Cursor(0);				
					break;
				}
				dstate=lose;
				break;		
		case write:if((numplay==2&&s3==0 && (cnt==posb2||cnt==posb2+1))||(s1==0 && cnt == pos1) || (s2==0 && cnt == pos2) || (pmark == 1&&((s1==0&&(cnt==pos1||cnt==pos1+1)) || (s2==0&&(cnt == pos2 || cnt==pos2+1))))){pause =1;
				yeet_time=0;lose_time=0;cnt1=13;task5.period =50;dstate=yeet;yeet_index=0;

				break;
				}
			if (menu ==1){dstate = mmenu;numplay=1;cnt1=13;task5.period=100;endcnt=0;cnt2 = 16;index=0;saved=0;posc=17;LCD_ClearScreen();LCD_Cursor(0);fp = 0;
					task2.period = 300;task3.period=300;					
					pause=1;break;
			} 
			if (dtime >=5000) {if(task3.period > 100){dtime=0;task2.period -=10;task3.period-=10;}}
			dstate=write;break;
		case yeet: if (yeet_time > 3000) {fp=0;etime=0;lose_time=0;menu=1;dstate=lose;break;}
				dstate = yeet;break;
		}
	switch(dstate) {
		case start2:break;
...
void clock() {
		...
		else {lose_time+=gcd;yeet_time+=gcd;}
}
int main(void) {
